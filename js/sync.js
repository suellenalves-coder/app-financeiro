...
// Duas chamadas sobrepostas (ex.: o foco da janela e o debounce de salvamento
// disparando quase juntos) reaproveitam a MESMA sincronização em andamento, em vez
// de rodar duas em paralelo — evita que uma pise no resultado da outra.
let syncInFlight = null;
export function syncNow() {
  if (!isLoggedIn()) return Promise.resolve({ changed: false });
...
// IMPORTANTE: precisa passar por syncNow() (baixa + mescla + envia), nunca por um
// pushNow() direto — um push "cru" sobrescreve a nuvem inteira com a cópia local,
// apagando qualquer alteração que outro aparelho tenha enviado enquanto este estava
// com dados desatualizados (ex.: sem ter reaberto o app/recuperado o foco).
let pushTimer = null;
...
function schedulePush() {
  ...
  pushTimer = setTimeout(() => {
    syncNow()
      .then(r => { if (r.changed && onRemoteChangeCb) onRemoteChangeCb(); })
      .catch(() => { /* estado de erro já registrado dentro de syncNow */ });
  }, 2000);
}
